...
export async function createApprovalInstance({
  ...
  submitterUserId,
  summary,
}: CreateInstanceArgs): Promise<mongoose.Types.ObjectId> {
  await connectToDatabase();
  ...
  return instance._id as mongoose.Types.ObjectId;
}
...
async function notifyStep(
  ...
  const employee = await Employee.findById(employeeId)
    ...
    .lean<{ name: string; divisionId?: mongoose.Types.ObjectId; branchId?: mongoose.Types.ObjectId } | null>();
...
export async function cancelInstance(instanceId: mongoose.Types.ObjectId | string, userId: string) {
  await connectToDatabase();
  const instance = await ApprovalInstance.findById(instanceId);
  if (!instance) return;
  instance.status = "rejected";
