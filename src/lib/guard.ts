...
export function scopeFilter(
  ctx: GuardContext,
  fields: { employee?: string; branch?: string; division?: string } = {}
  ...
  switch (ctx.permission.scope) {
    case "all":
      return {};
    case "branch":
      return fields.branch && ctx.user.branchId ? { [fields.branch]: ctx.user.branchId } : {};
    case "division":
      return fields.division && ctx.user.divisionId ? { [fields.division]: ctx.user.divisionId } : {};
    case "self":
    default:
      return { [employeeField]: ctx.user.employeeId ?? "000000000000000000000000" };
  }
}
