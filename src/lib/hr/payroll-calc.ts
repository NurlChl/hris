...
export async function calculatePayroll(employeeId: string, period: string): Promise<PayrollCalculation> {
  ...
  const logs = await Attendance.find({ employeeId, date: { $gte: monthStart, $lte: monthEnd } }).lean<
    Array<{ date: Date; isLate: boolean; lateMinutes: number }>
  >();
  ...
  const absentKeys = elapsedWorkingDays.filter((k) => !presentKeys.has(k) && !leaveKeys.has(k));

  let latePenalty = profile?.exemptLatePenalty ? 0 : lateMinutes * rates.latePerMinute;
  if (rates.latePenaltyCap > 0) latePenalty = Math.min(latePenalty, rates.latePenaltyCap);
  const absentPenalty = profile?.exemptAbsentPenalty ? 0 : absentKeys.length * rates.absentPerDay;
