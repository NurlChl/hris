...
export async function resolveRecipientsByUserIds(userIds: Array<string | mongoose.Types.ObjectId>) {
  await connectToDatabase();
  const users = await User.find({ _id: { $in: userIds }, isActive: { $ne: false } })
    .select("email phone employeeId")
    .lean<Array<{ _id: mongoose.Types.ObjectId; email: string; phone?: string; employeeId?: mongoose.Types.ObjectId }>>();
  ...
  const employees = employeeIds.length
    ? await Employee.find({ _id: { $in: employeeIds } })
        ...
        .lean<Array<{ _id: mongoose.Types.ObjectId; name: string; personalEmail?: string; phone?: string }>>()
    : [];
...
export async function resolveRecipientForEmployee(employeeId: string | mongoose.Types.ObjectId) {
  await connectToDatabase();
  const user = await User.findOne({ employeeId, isActive: { $ne: false } }).select("_id").lean<{ _id: mongoose.Types.ObjectId } | null>();
  if (!user) return [];
...
export async function resolveRecipientsByRole(
  ...
  await connectToDatabase();
  const role = await Role.findOne({ name: roleName }).select("_id").lean<{ _id: mongoose.Types.ObjectId } | null>();
  if (!role) return [];

  const users = await User.find({ roleId: role._id, isActive: { $ne: false } })
    .select("_id employeeId")
    .lean<Array<{ _id: mongoose.Types.ObjectId; employeeId?: mongoose.Types.ObjectId }>>();
  ...
  if (opts.divisionId || opts.branchId) {
    ...
    const scoped = await Employee.find(scopeQuery).select("_id").lean<Array<{ _id: mongoose.Types.ObjectId }>>();
