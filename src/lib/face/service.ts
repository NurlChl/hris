...
export async function getFaceSettings(): Promise<FaceSettings> {
  ...
  return {
    enabled: Boolean(s.face_recognition_enabled),
    strictness,
...
export async function verifyAttendanceFace(
  ...
  const profile = await FaceProfile.findOne({ employeeId: new mongoose.Types.ObjectId(employeeId) })
    .select("descriptors")
    .lean<{ _id: mongoose.Types.ObjectId; descriptors: string } | null>();
  ...
  void FaceProfile.updateOne({ _id: profile._id }, { lastVerifiedAt: new Date() }).exec();
