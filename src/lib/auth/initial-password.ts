...
export async function readPolicy(): Promise<Record<string, RolePolicy>> {
  const [stored, roles, settings] = await Promise.all([
    readStored(),
    Role.find({ name: { $nin: UNCONFIGURABLE_ROLES } }).select("name").lean<Array<{ name: string }>>(),
    getSettings(),
  ]);
  // Deployments that predate per-role passwords used one shared value; it
  // remains the starting point for every role until someone changes it.
  const legacy = String(settings.default_employee_password ?? "");
  ...
  for (const { name } of roles) {
    ...
    out[name] = entry
      ...
      : { mode: legacy ? "fixed" : "random", password: legacy };
  }
  return out;
}
