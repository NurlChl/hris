...
import { requirePermission, requireUser, parseBody, pagination, scopeFilter, BadRequest, Conflict, NotFound } from "@/lib/guard";
...
export const GET = wrapRouteHandler(async (req) => {
  ...
  const filter: Record<string, unknown> = {
    ...scopeFilter(ctx, { employee: "employeeId" }),
  };
  ...
  if (employeeId && objectId.safeParse(employeeId).success) filter.employeeId = new mongoose.Types.ObjectId(employeeId);

  const q = sp.get("q")?.trim();
  if (q) {
    const rx = new RegExp(escapeRegex(q.slice(0, 60)), "i");
    const ids = await Employee.find({ $or: [{ name: rx }, { employeeId: rx }] }).select("_id").limit(500).lean<Array<{ _id: mongoose.Types.ObjectId }>>();
...
export const POST = wrapRouteHandler(async (req) => {
  ...
  const employee = await Employee.findById(body.employeeId)
    ...
    .lean<{ _id: mongoose.Types.ObjectId; name: string; positionId?: { name?: string } | null } | null>();
  ...
  if (previous) {
    previous.nextContractId = contract._id as mongoose.Types.ObjectId;
    previous.decision = body.type === "pkwtt" ? "permanent" : "renew";
    previous.decidedAt = new Date();
    previous.decidedBy = new mongoose.Types.ObjectId(ctx.user.id);
    await previous.save();
  }
