...
import { requirePermission, requireUser, parseBody, BadRequest, Conflict, Forbidden, NotFound } from "@/lib/guard";
...
export const GET = wrapRouteHandler<Ctx>(async (req, { params }) => {
  ...
  const contract = await Contract.findById(id)
    ...
    .lean<Record<string, unknown> & { employeeId: { _id: mongoose.Types.ObjectId } | null; status: string } | null>();
...
export const PATCH = wrapRouteHandler<Ctx>(async (req, { params }) => {
  ...
  if (body.templateId !== undefined) {
    contract.generatedFromTemplateId = body.templateId ? new mongoose.Types.ObjectId(body.templateId) : null;
...
export const POST = wrapRouteHandler<Ctx>(async (req, { params }) => {
  ...
  switch (body.action) {
    ...
    case "not_renew": {
      ...
      contract.decidedBy = new mongoose.Types.ObjectId(ctx.user.id);
