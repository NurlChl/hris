...
export const POST = wrapRouteHandler(async (req) => {
  ...
  const [template, employee] = await Promise.all([
    ...
    Employee.findById(body.employeeId)
      ...
      .lean<{ name: string; divisionId?: mongoose.Types.ObjectId; branchId?: mongoose.Types.ObjectId; status: string } | null>(),
  ]);
...
export const PATCH = wrapRouteHandler(async (req) => {
  ...
  evaluation.finalizedBy = ctx.user.id as unknown as mongoose.Types.ObjectId;
