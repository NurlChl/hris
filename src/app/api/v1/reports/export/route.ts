...
import { requireUser, BadRequest, Forbidden } from "@/lib/guard";
...
export const GET = wrapRouteHandler(async (req) => {
  ...
  const { rows, headers } = await buildRows(dataset, start, end, perm.scope, ctx.user.branchId);
...
async function buildRows(
  ...
  const branchFilter = scope === "branch" && branchId ? { branchId } : {};

  if (dataset === "attendance") {
    const logs = await Attendance.find({ date: { $gte: start, $lte: end } })
  ...
  if (dataset === "corrections") {
    const rows = await AttendanceCorrection.find({ date: { $gte: start, $lte: end } })
      .populate("employeeId", "name employeeId")
      .sort({ date: 1 })
      .limit(20000)
      .lean();
  ...
  return {
    ...
    rows: employees.map((e) => [
      ...
      // Sensitive fields are stored encrypted; an authorised export decrypts them.
      e.npwp ? decrypt(e.npwp as string) : "",
