...
async function forfeitUnworkedHolidaySwaps(todayKey: string) {
  ...
  const candidates = await HolidaySwapRequest.find({
    ...
    .limit(500)
    .lean<Array<{ _id: mongoose.Types.ObjectId; employeeId: mongoose.Types.ObjectId; holidayDate: Date }>>();
...
async function recordHolidayOvertime(todayKey: string) {
  ...
  const logs = await Attendance.find({
    ...
    clockIn: { $ne: null },
    clockOut: { $ne: null },
  }).lean<Array<{ employeeId: mongoose.Types.ObjectId; clockIn: Date; clockOut: Date }>>();
...
async function sendContractReminders() {
  ...
  for (const days of windows) {
    ...
    const contracts = await Contract.find({
      ...
      .limit(200)
      .lean<Array<{ _id: mongoose.Types.ObjectId; employeeId: mongoose.Types.ObjectId; endDate: Date; type: string }>>();

    for (const contract of contracts) {
      const employee = await Employee.findById(contract.employeeId)
        .select("name divisionId")
        .lean<{ name: string; divisionId?: mongoose.Types.ObjectId } | null>();
...
async function sendBirthdayGreetings() {
  ...
  const birthdayPeople = await Employee.aggregate<{ _id: mongoose.Types.ObjectId; name: string }>([
